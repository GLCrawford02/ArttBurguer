...
import { Funcionario, Tarefa } from '../types';
import { Bot, Loader2, Sparkles, Plus, Trash2, Calendar, MessageSquare, Briefcase, CheckCircle, AlertTriangle, UserCircle, Save, Plane, Clock, Power, ShieldOff, ShieldCheck, Fingerprint, MapPin, CheckSquare, ChevronUp, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden bg-white">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center p-4 hover:bg-gray-50 transition-colors">
        <h4 className="font-bold text-gray-700 flex items-center gap-2">{icon} {title}</h4>
        {isOpen ? <ChevronUp size={20} className="text-gray-500" /> : <ChevronDown size={20} className="text-gray-500" />}
      </button>
      {isOpen && (
        <div className="p-4 border-t border-gray-100 animate-in fade-in duration-200">
          {children}
        </div>
      )}
    </div>
...
export default function GestaoEquipeManager({ activeView = 'gestao' }: { activeView?: 'gestao' | 'ia' }) {
  ...
  useEffect(() => {
    ...
    const unsubTarefas = onValue(ref(db, 'tarefas'), snap => {
      if (snap.val()) {
        const list = Object.entries(snap.val()).map(([id, val]: any) => ({ id, ...val }));
        setTarefas(list);
      } else {
    ...
    return () => { unsubFunc(); unsubGestao(); unsubTarefas(); };
  }, []);
  ...
  const tarefasDoFuncionario = tarefas.filter(t => 
    t.status === 'pendente' && 
    (t.responsaveisIds?.includes(selectedFuncId || '') || t.responsavelId === selectedFuncId)
  ...
  const funcionariosGerenciaveis = funcionarios.filter(f => {
    const cargos = Array.isArray(f.cargo) ? f.cargo : [f.cargo || 'Atendente'];
    const isManagement = cargos.every(c => ['Administrador', 'Gerente', 'Dono', 'TI'].includes(c));
    const isKds = cargos.some(c => (c || '').toUpperCase().includes('KDS'));
    return !isManagement && !isKds;
  });
        ...
        {selectedFuncId && selectedFunc ? (
          <div className="mt-6 space-y-6">
            <div className="space-y-4">
              <AccordionSection title="Atribuições Pendentes" icon={<CheckSquare size={18} className="text-purple-600" />}>
                {tarefasDoFuncionario.length === 0 ? (
                  <p className="text-sm text-gray-400 italic text-center py-4">Nenhuma tarefa pendente.</p>
                ) : (
                  <div className="space-y-2">
                    {tarefasDoFuncionario.map(t => (
                      <div key={t.id} className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                        <p className="font-bold text-sm text-gray-800">{t.titulo}</p>
                        <p className="text-xs text-gray-500">{t.dataAgendada ? `Para ${t.dataAgendada.split('-').reverse().join('/')}` : 'Sem data'}</p>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <AccordionSection title="Desempenho e Faltas" icon={<Briefcase size={18} className="text-blue-600" />}>
              ...
              <AccordionSection title="Horários e Ponto" icon={<Clock size={18} className="text-indigo-600" />}>
                <div className="space-y-6 animate-in fade-in">
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
                      <div className="p-4 text-center"><p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Horas Trabalhadas</p><p className="text-2xl font-black text-blue-600">{minParaStr(totalMinTrab)}</p><p className="text-[10px] text-gray-400 mt-0.5">{pontosFiltrados.length} dia(s) registrado(s)</p></div>
                      <div className="p-4 text-center"><p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Horas Esperadas</p><p className="text-2xl font-black text-gray-600">{minParaStr(totalMinEsp)}</p><p className="text-[10px] text-gray-400 mt-0.5">Conforme quadro de horários</p></div>
                      <div className="p-4 text-center"><p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Saldo</p><p className={`text-2xl font-black ${saldoMin >= 0 ? 'text-green-600' : 'text-red-500'}`}>{saldoMin >= 0 ? '+' : '-'}{minParaStr(saldoMin)}</p><p className={`text-[10px] mt-0.5 font-bold ${saldoMin >= 0 ? 'text-green-500' : 'text-red-400'}`}>{saldoMin >= 0 ? 'Horas extras' : 'Horas devendo'}</p></div>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b border-gray-100"><tr><th className="text-left px-4 py-2.5 text-[10px] font-bold text-gray-400 uppercase">Data</th><th className="text-center px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase">Chegada</th><th className="text-center px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase">S. Almoço</th><th className="text-center px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase">V. Almoço</th><th className="text-center px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase">Saída</th><th className="text-center px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase">Trabalhado</th><th className="text-center px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase">Esperado</th><th className="text-center px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase">Saldo</th><th className="px-3 py-2.5"></th></tr></thead>
                        <tbody className="divide-y divide-gray-50">
                          {pontosFiltrados.map(p => {
                            const date = getDataPonto(p); const minTrab = calcMinTrabalhados(p); const minEsp = calcMinEsperados(date); const saldo = minTrab - minEsp; const chegada = p.chegada?.hora || p.entrada || '—'; const saidaAlm = p.saida_almoco?.hora || '—'; const voltaAlm = p.volta_almoco?.hora || '—'; const saida = p.saida_final?.hora || p.saida || '—'; const diasSemana = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb']; const dataLabel = `${diasSemana[date.getDay()]}, ${date.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}`;
                            return (<tr key={p.id} className="hover:bg-gray-50 transition-colors"><td className="px-4 py-3"><span className="font-bold text-gray-800 text-sm">{dataLabel}</span></td><td className="px-3 py-3 text-center"><div className="flex flex-col items-center gap-0.5"><span className={`font-bold text-xs px-2 py-1 rounded ${chegada !== '—' ? 'bg-green-100 text-green-700' : 'text-gray-300'}`}>{chegada}</span>{p.chegada?.lat && <a href={`https://maps.google.com/maps?q=${p.chegada.lat},${p.chegada.lng}`} target="_blank" rel="noopener noreferrer" className="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-0.5 font-medium"><MapPin size={9}/> ver local</a>}</div></td><td className="px-3 py-3 text-center"><div className="flex flex-col items-center gap-0.5"><span className={`font-bold text-xs px-2 py-1 rounded ${saidaAlm !== '—' ? 'bg-orange-100 text-orange-700' : 'text-gray-300'}`}>{saidaAlm}</span>{p.saida_almoco?.lat && <a href={`https://maps.google.com/maps?q=${p.saida_almoco.lat},${p.saida_almoco.lng}`} target="_blank" rel="noopener noreferrer" className="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-0.5 font-medium"><MapPin size={9}/> ver local</a>}</div></td><td className="px-3 py-3 text-center"><div className="flex flex-col items-center gap-0.5"><span className={`font-bold text-xs px-2 py-1 rounded ${voltaAlm !== '—' ? 'bg-blue-100 text-blue-700' : 'text-gray-300'}`}>{voltaAlm}</span>{p.volta_almoco?.lat && <a href={`https://maps.google.com/maps?q=${p.volta_almoco.lat},${p.volta_almoco.lng}`} target="_blank" rel="noopener noreferrer" className="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-0.5 font-medium"><MapPin size={9}/> ver local</a>}</div></td><td className="px-3 py-3 text-center"><div className="flex flex-col items-center gap-0.5"><span className={`font-bold text-xs px-2 py-1 rounded ${saida !== '—' ? 'bg-red-100 text-red-700' : 'text-gray-300'}`}>{saida}</span>{p.saida_final?.lat && <a href={`https://maps.google.com/maps?q=${p.saida_final.lat},${p.saida_final.lng}`} target="_blank" rel="noopener noreferrer" className="text-[10px] text-blue-500 hover:text-blue-700 flex items-center gap-0.5 font-medium"><MapPin size={9}/> ver local</a>}</div></td><td className="px-3 py-3 text-center font-black text-blue-700">{minTrab > 0 ? minParaStr(minTrab) : '—'}</td><td className="px-3 py-3 text-center text-gray-500 font-medium">{minEsp > 0 ? minParaStr(minEsp) : '—'}</td><td className="px-3 py-3 text-center">{minTrab > 0 && minEsp > 0 ? (<span className={`font-black text-xs px-2 py-1 rounded-full ${saldo >= 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>{saldo >= 0 ? '+' : '-'}{minParaStr(saldo)}</span>) : <span className="text-gray-300">—</span>}</td><td className="px-3 py-3 text-right"><button onClick={() => removePonto(p.id)} className="text-red-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded transition-colors"><Trash2 size={13}/></button></td></tr>);
                          })}
                        </tbody>
                      </table>
                      {pontosFiltrados.length === 0 && (<div className="py-12 text-center text-gray-400"><Clock size={32} className="mx-auto mb-2 opacity-30" /><p className="text-sm">Nenhum registro de ponto no período selecionado.</p></div>)}
                    </div>
                  </div>
                </div>
              </AccordionSection>

              <AccordionSection title="Status e Desligamento" icon={<UserCircle size={18} className="text-red-600" />}>
