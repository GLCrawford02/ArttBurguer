  ...
  const salvarConta = async () => {
    ...
    if (!itemEdit && tipoLancamento === 'Parcelado') {
      ...
      for(let i=inicial; i<=parcelas; i++) {
         ...
         const recData: any = { descricao: `${descricao} (${i}/${parcelas})`, valor: valFinal, vencimento: dataStr, status: statusFinal, recorrencia: 'Nenhuma', fimRecorrencia: '' };
         ...
         if (gerarLembrete) {
            await set(push(ref(db, 'tarefas')), {
              ...
              responsaveisIds: [responsavelLembrete], dataAgendada: dataStr, horaAgendada: horaLembrete, prioridade: 'Alta', categoria: 'Financeiro', recorrencia: 'Nenhuma', status: 'pendente', notificadoWhatsApp: false, timestamp: Date.now()
            });
         }
      }
      ...
      const data: any = { descricao, valor: total, vencimento, status: statusFinal, recorrencia: itemEdit ? recorrencia : rec, fimRecorrencia };
      ...
      if (itemEdit) {
        ...
        if (gerarLembrete) {
          await set(push(ref(db, 'tarefas')), {
            ...
            responsaveisIds: [responsavelLembrete], dataAgendada: vencimento, horaAgendada: horaLembrete, prioridade: 'Alta', categoria: 'Financeiro', recorrencia: rec, status: 'pendente', notificadoWhatsApp: false, timestamp: Date.now()
          });
        }
    ...
    setItemEdit(null);
    setDescricao(''); setValor(''); setVencimento(''); setGerarLembrete(false);
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-fit space-y-4">
              ...
              {tipoLancamento === 'Parcelado' && !itemEdit ? (
                <div className="w-full space-y-2">
                  <div className="flex gap-4"><label className="flex items-center text-xs font-bold cursor-pointer"><input type="radio" checked={modoParcelamento === 'total'} onChange={() => setModoParcelamento('total')} className="mr-1" /> Valor Total</label><label className="flex items-center text-xs font-bold cursor-pointer"><input type="radio" checked={modoParcelamento === 'parcela'} onChange={() => setModoParcelamento('parcela')} className="mr-1" /> Valor Parcela</label></div>
                  <div className="flex gap-2 w-full"><div className="relative flex-1"><span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 text-sm">R$</span><input type="text" disabled={modoParcelamento === 'parcela'} value={modoParcelamento === 'total' ? (valor === '' ? '' : Number(valor).toLocaleString('pt-BR', { minimumFractionDigits: 2 })) : (valorParcela === '' ? '' : (Number(valorParcela) * Number(qtdParcelas)).toLocaleString('pt-BR', { minimumFractionDigits: 2 }))} onChange={e => { const d = e.target.value.replace(/\D/g, ''); setValor(d ? (parseInt(d, 10) / 100).toString() : ''); }} className="w-full pl-8 pr-2 py-2 border border-gray-200 rounded-lg outline-none bg-white" /><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-gray-500">Valor Total</label></div><div className="relative flex-1"><span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 text-sm">R$</span><input type="text" disabled={modoParcelamento === 'total'} value={modoParcelamento === 'parcela' ? (valorParcela === '' ? '' : Number(valorParcela).toLocaleString('pt-BR', { minimumFractionDigits: 2 })) : (valor === '' ? '' : (Number(valor) / (Number(qtdParcelas) || 1)).toLocaleString('pt-BR', { minimumFractionDigits: 2 }))} onChange={e => { const d = e.target.value.replace(/\D/g, ''); setValorParcela(d ? (parseInt(d, 10) / 100).toString() : ''); }} className="w-full pl-8 pr-2 py-2 border border-gray-200 rounded-lg outline-none bg-white" /><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-gray-500">Valor Parcela</label></div></div>
                </div>
              ) : (
                <div className="relative w-full"><span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-500 text-sm">R$</span><input type="text" placeholder="0,00" value={valor === '' ? '' : Number(valor).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} onChange={e => { const d = e.target.value.replace(/\D/g, ''); setValor(d ? (parseInt(d, 10) / 100).toString() : ''); }} className={`w-full pl-8 pr-2 py-2 border border-gray-200 rounded-lg outline-none focus:ring-2 ${tipoConta === 'pagar' ? 'focus:ring-red-500' : 'focus:ring-blue-500'}`} /></div>
              )}
              ...
              {tipoConta === 'receber' && (
                <select value={statusReceber} onChange={e=>setStatusReceber(e.target.value as any)} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-blue-500"><option value="Pendente">Pendente</option><option value="Recebido">Recebido</option></select>
              )}

              {isAdminOrDono && (
                <div className="space-y-2 mt-2">
                  {!itemEdit ? (
                    <div className="flex space-x-2 items-center"><select value={tipoLancamento} onChange={e => setTipoLancamento(e.target.value as any)} className={`w-1/3 p-2 border border-gray-200 rounded-lg outline-none focus:ring-2 ${tipoConta === 'pagar' ? 'focus:ring-red-500' : 'focus:ring-blue-500'}`}><option value="Unico">Pagamento Único</option><option value="Parcelado">Parcelado</option><option value="Recorrente">Recorrente</option></select>{tipoLancamento === 'Parcelado' && (<div className="flex gap-2 items-center flex-1"><div className="relative flex-1"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-gray-500">Inicia</label><input type="number" min="1" max={qtdParcelas} value={parcelaInicial} onChange={e => setParcelaInicial(e.target.value)} className="w-full p-2 border border-gray-200 rounded-lg" /></div><span className="text-gray-400 font-bold">de</span><div className="relative flex-1"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-gray-500">Total</label><input type="number" min="2" max="120" value={qtdParcelas} onChange={e => setQtdParcelas(e.target.value)} className="w-full p-2 border border-gray-200 rounded-lg" /></div></div>)}{tipoLancamento === 'Recorrente' && (<select value={recorrencia} onChange={e => setRecorrencia(e.target.value as any)} className="flex-1 p-2 border border-gray-200 rounded-lg"><option value="Nenhuma">Selecione...</option><option value="Diária">Diária</option><option value="Semanal">Semanal</option><option value="Mensal">Mensal</option><option value="Anual">Anual</option></select>)}</div>
                  ) : (
                    <div className="grid grid-cols-2 gap-2"><div className="relative"><select value={recorrencia} onChange={e=>setRecorrencia(e.target.value as any)} className="w-full p-2 pl-8 border border-gray-200 rounded-lg"><option value="Nenhuma">Sem Recorrência</option><option value="Diária">Diária</option><option value="Semanal">Semanal</option><option value="Mensal">Mensal</option><option value="Anual">Anual</option></select><Repeat size={16} className="absolute left-2.5 top-1/2 transform -translate-y-1/2 text-gray-500" /></div>{recorrencia !== 'Nenhuma' && (<div className="relative"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-gray-500">Parar Em:</label><input type="date" value={fimRecorrencia} onChange={e=>setFimRecorrencia(e.target.value)} className="w-full p-2 border border-gray-200 rounded-lg" /></div>)}</div>
                  )}
                  {(tipoLancamento === 'Recorrente' || itemEdit) && recorrencia !== 'Nenhuma' && !itemEdit && (<div className="relative mt-2"><label className="absolute -top-2 left-2 bg-white px-1 text-[10px] font-bold text-gray-500">Parar Em (Opcional):</label><input type="date" value={fimRecorrencia} onChange={e=>setFimRecorrencia(e.target.value)} className="w-full p-2 border border-gray-200 rounded-lg" /></div>)}
                </div>
              )}
            ...
            <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
               <table className="w-full text-left min-w-[500px]">
                 <thead className="bg-gray-50 border-b border-gray-100 text-xs text-gray-500 uppercase"><tr><th className="p-4">Descrição</th><th className="p-4">Valor</th><th className="p-4">Vencimento</th><th className="p-4">Status</th><th className="p-4 text-right">Ações</th></tr></thead>
                 {loading ? (
                 ...
                 <tbody className="divide-y divide-gray-50 text-sm">
                   {contas.sort((a: any,b: any)=>a.vencimento.localeCompare(b.vencimento)).map((c: any) => (
                     <tr key={c.id} className="hover:bg-gray-50">
                       <td className="p-4 font-medium text-gray-800">{c.descricao} {c.recorrencia && c.recorrencia !== 'Nenhuma' && <span className="ml-2 text-[10px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded-full"><Repeat size={10} className="inline mr-1"/>{c.recorrencia}{c.fimRecorrencia ? ` até ${formatarData(c.fimRecorrencia)}` : ''}</span>} {tipoConta === 'pagar' && <span className="block text-xs text-gray-400">{c.tipo} {c.fornecedorId ? `| ${fornecedores.find((f:any)=>f.id===c.fornecedorId)?.nome}` : ''}</span>}</td>
                       ...
                       <td className="p-4"><button onClick={()=>alternarStatusLocal(c.id, c.status.includes('P') ? (tipoConta === 'pagar' ? 'Pago' : 'Recebido') : 'Pendente')} className={`px-2 py-1 rounded-full text-xs font-bold ${!c.status.includes('Pendente') ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{c.status}</button></td>
                   ...
                   {contas.length === 0 && <tr><td colSpan={5} className="p-4 text-center text-gray-400">Nenhum registro.</td></tr>}
                 </tbody>
                 )}
               </table>
            </div>
